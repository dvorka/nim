...
%----------------------------------------------------------------------------
% strategy(+Stav,+0,Pocet_k*(min+max)+min+s,+P,+R,-Je_vyhr,-Body)
strategy([],B,C,_,_,V,OB):-OB is (B+(C//2)*2),
                           ((var(V),sude(C),V=1);
                            (var(V),liche(C),V=0);
                            (nonvar(V))).
strategy([H|T],B,C2,P,R,V,OB):-H<P,MB is B+1,strategy(T,MB,C2,P,R,V,OB).
strategy([H|T],B,C2,P,R,V,OB):-H>=P,
                               C is (H mod (P+R)),C =< (P-1),
                               MB is B+1,
                               strategy(T,MB,C2,P,R,V,OB).
strategy([H|T],B,C2,P,R,V,OB):-H>=P,C is (H mod (P+R)),
                               C =< (P+P-1),C >= P,
                               C2O is C2+1,
                               strategy(T,B,C2O,P,R,V,OB).
strategy([H|T],B,C2,P,R,V,OB):-H>=P,C is (H mod (P+R)),C > (P+P-1),
                               ((var(V),V=0);(nonvar(V))),
                               strategy(T,B,C2,P,R,V,OB).
%----------------------------------------------------------------------------
...
% na vyhru ve hre : check(+Moznosti,+P,+R,-Opt.Pos)
check([],_,_,[]).
check([H|T],P,R,V):-strategy(H,0,0,P,R,C,_),
                    ((C=0,!,check(T,P,R,V));
                     (C=1,!,V=H)).
%----------------------------------------------------------------------------%
% vybere ze seznamu stavy , ze kterych nejde hrat na vyhravajici pozici,
% a ohodnoti je : projdi(+Stavy,K,P,R,-Oh_Tahy)
projdi([],_,_,_,[]).
projdi([H|T],K,P,R,V):-moznosti(H,K,P,R,Moz),
                       check(Moz,P,R,Out),
                       ((Out=[],              % z H nejde hrat na strategii
                         projdi(T,K,P,R,MV),
                         strategy(H,0,0,P,R,_,Bod),
                         V=[[H,Bod]|MV]);
                        (Out\=[],projdi(T,K,P,R,V))).
%----------------------------------------------------------------------------%
% vybere nejlepe ohodnoceny stav : nej(+Oh_stavy,Pomoc_opt,-Nej)
nej([],[O|_],O).
nej([[S,B]|T],[OS,OB],O):-((B>OB,nej(T,[S,B],O));
                           (B=<OB,nej(T,[OS,OB],O))).
%----------------------------------------------------------------------------%
% vybere stav ze ktereho nejde hrat na vyhr. pos ( pokud to jde ) a zaroven
% smeruje nejvice k strategii : vyber(+Moznosti,+K,+P,+R,-Optimum)
vyber(Moz,K,P,R,Opt):-projdi(Moz,K,P,R,Tahy),
                      ((Tahy=[],Opt=[]);
                       (Tahy\=[],[Zac|Pom]=Tahy,nej(Pom,Zac,Opt))).
%----------------------------------------------------------------------------%
